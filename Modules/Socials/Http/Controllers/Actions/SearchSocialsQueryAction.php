...
class SearchSocialsQueryAction
{
    public function execute(User $auth_user, Request $request)
    {
        // search socials
        $socials = (new Social)->newQuery();
        ...
        if ($request->input('created_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $socials = $socials->where('created_at', '>=', $from)->where('created_at', '<=', $to);
                } elseif ($from) {
                    $socials = $socials->where('created_at', '>=', $from);
                } elseif ($to) {
                    $socials = $socials->where('created_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
        ...
        if ($request->input('last_updated_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $socials = $socials->where('updated_at', '>=', $from)->where('updated_at', '<=', $to);
                } elseif ($from) {
                    $socials = $socials->where('updated_at', '>=', $from);
                } elseif ($to) {
                    $socials = $socials->where('updated_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
