...
class SearchBranchesQueryAction
{
    public function execute(User $auth_user, Request $request)
    {
        // Search branches
        $branches = (new Branch)->newQuery();
        ...
        if ($request->input('created_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $branches = $branches->where('created_at', '>=', $from)->where('created_at', '<=', $to);
                } elseif ($from) {
                    $branches = $branches->where('created_at', '>=', $from);
                } elseif ($to) {
                    $branches = $branches->where('created_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
        ...
        if ($request->input('last_updated_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $branches = $branches->where('updated_at', '>=', $from)->where('updated_at', '<=', $to);
                } elseif ($from) {
                    $branches = $branches->where('updated_at', '>=', $from);
                } elseif ($to) {
                    $branches = $branches->where('updated_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
