...
class SearchContactsQueryAction
{
    public function execute(User $auth_user, Request $request)
    {
        ...
        if ($request->input('created_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $contacts = $contacts->where('created_at', '>=', $from)->where('created_at', '<=', $to);
                } elseif ($from) {
        ...
        if ($request->input('last_updated_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $contacts = $contacts->where('updated_at', '>=', $from)->where('updated_at', '<=', $to);
                } elseif ($from) {
                    $contacts = $contacts->where('updated_at', '>=', $from);
                } elseif ($to) {
                    $contacts = $contacts->where('updated_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
