...
class SearchFooterLinksQueryAction
{
    public function execute(User $auth_user, Request $request)
    {
        // Search footer link
        $footer_links = (new FooterLink)->newQuery();
        ...
        if ($request->input('created_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $footer_links = $footer_links->where('created_at', '>=', $from)->where('created_at', '<=', $to);
                } elseif ($from) {
                    $footer_links = $footer_links->where('created_at', '>=', $from);
                } elseif ($to) {
                    $footer_links = $footer_links->where('created_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
        ...
        if ($request->input('last_updated_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $footer_links = $footer_links->where('updated_at', '>=', $from)->where('updated_at', '<=', $to);
                } elseif ($from) {
                    $footer_links = $footer_links->where('updated_at', '>=', $from);
                } elseif ($to) {
                    $footer_links = $footer_links->where('updated_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
