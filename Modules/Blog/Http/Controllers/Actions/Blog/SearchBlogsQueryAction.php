    ...
    public function execute(User $auth_user, Request $request)
    {
        ...
        if ($request->input('created_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $blogs = $blogs->where('created_at', '>=', $from)->where('created_at', '<=', $to);
                } elseif ($from) {
                    $blogs = $blogs->where('created_at', '>=', $from);
                } elseif ($to) {
                    $blogs = $blogs->where('created_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
        ...
        if ($request->input('last_updated_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $blogs = $blogs->where('updated_at', '>=', $from)->where('updated_at', '<=', $to);
                } elseif ($from) {
                    $blogs = $blogs->where('updated_at', '>=', $from);
                } elseif ($to) {
                    $blogs = $blogs->where('updated_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
